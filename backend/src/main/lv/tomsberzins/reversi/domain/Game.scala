...
import lv.tomsberzins.reversi.domain.GameState.{
  Board,
  getFreePositions,
  resetBoard
}
...
object GameState {
  ...
  def attemptMove(
    ...
    @tailrec
    def getFlippableCount(
        board: Board,
        position: Position,
        howToCalculateNextPosition: Position => Position,
        amountFlipped: Int = 0
    ): Int = {
      val positionBeingFlipped = howToCalculateNextPosition(position)
      val enclosingPositionOfFlipped = howToCalculateNextPosition(
        positionBeingFlipped
      )
    ...
    def flipStones(
      ...
      if (amountToFlip <= 0) {
        ...
        board.get(positionToBeFlipped) match {
          case Some(Some(stone)) =>
            val updatedBoard =
              board.updated(positionToBeFlipped, stone.flip().some)
            flipStones(
              updatedBoard,
              positionToBeFlipped,
              amountToFlip - 1,
    ...
    def flipIfPossible(
        board: Board,
        toBePlaced: Position,
        howToCalculateNextPositions: List[Position => Position]
    ): Board = {
    ...
    lazy val newBoard = flipIfPossible(
      gameState.board,
      toBePlaced.position,
      List(
        flipHorizontalRight,
...
case class Game private (
  ...
  def move(stone: Stone, position: Position): Either[String, Game] = {
    for {
      updatedGameState <- GameState.attemptMove(
        Move(position, stone),
        gameState
      )
  ...
  /**
   * Check if provided stone can be placed anywhere on the board and flip at least one opposing stone
   */
...
object Game {
  ...
  implicit val encodeGameList: Encoder[List[Game]] = (games: List[Game]) => {
    val jsonObjects = for {
      game <- games
    } yield {
      Json.obj(
        ("id", game.id.asJson),
        ("name", game.name.asJson),
        ("players", game.playerToStoneMap.asJson),
        ("owner", game.createdBy.asJson)
      )
    }

    Json.fromValues(jsonObjects)
  }
